...
const double DisplaySpacing = 43.0; // 五个诊断对象沿X方向间隔43毫米。
...
private:
    ...
    DiagnosticRenderMode m_renderMode = DiagnosticRenderMode::SmoothSpecular; // 默认使用最容易暴露摩尔纹的平滑高光模式。
...
class RenderDiagnosticWindow : public QMainWindow
{
public:
    explicit RenderDiagnosticWindow(MyVoxel::VoxelLevel maximumLevel, QWidget* parent = nullptr)
        : QMainWindow(parent)
        , m_viewer(new DiagnosticOpenGLWidget(this))
        , m_maximumLevel(maximumLevel)
    {
        setWindowTitle(QStringLiteral("MyVoxel VolumeField Render Diagnostic - Level %1").arg(static_cast<unsigned int>(maximumLevel)));
...
private:
    ...
    bool initializeScene()
    {
        try
        {
            ...
            const MyVoxel::Geometry::Mesh voxelMesh = MyVoxel::VoxelSurfaceMesher::build(voxelShape, DiagnosticColor);
            ...
            const MyVoxel::Geometry::Mesh standardMeshNormals = makeAreaWeightedNormalMesh(standardMesh);
            ...
            if (!referenceMesh.isRenderable() || !voxelMesh.isRenderable() || !standardMesh.isRenderable() ||
                !standardMeshNormals.isRenderable() || !featureMesh.isRenderable())
            {
                qCritical() << "At least one diagnostic mesh is not renderable.";
                return false;
            }

            m_referenceObject = m_viewer->addMesh(referenceMesh, translationMatrix(static_cast<float>(-2.0 * DisplaySpacing)));
            m_voxelObject = m_viewer->addMesh(voxelMesh, translationMatrix(static_cast<float>(-DisplaySpacing)));
            m_standardObject = m_viewer->addMesh(standardMesh, translationMatrix(0.0f));
            m_standardMeshNormalObject = m_viewer->addMesh(standardMeshNormals, translationMatrix(static_cast<float>(DisplaySpacing)));
            m_featureObject = m_viewer->addMesh(featureMesh, translationMatrix(static_cast<float>(2.0 * DisplaySpacing)));
            createToolBars();
            m_viewer->setIsometricView();
            updateStatus(QStringLiteral("平滑高光"));

            qDebug().noquote() << QStringLiteral("Level %1 | minimum voxel edge: %2 mm").arg(static_cast<unsigned int>(m_maximumLevel)).arg(voxelShape.grid().minimumCellEdgeLength(), 0, 'f', 6);
            qDebug().noquote() << QStringLiteral("Analytic reference: %1 vertices / %2 triangles").arg(static_cast<qulonglong>(referenceMesh.vertexCount())).arg(static_cast<qulonglong>(referenceMesh.triangleCount()));
            ...
            qDebug().noquote() << QStringLiteral("Standard gradient normals: %1 vertices / %2 triangles").arg(static_cast<qulonglong>(standardMesh.vertexCount())).arg(static_cast<qulonglong>(standardMesh.triangleCount()));
            qDebug().noquote() << QStringLiteral("Standard mesh normals: %1 vertices / %2 triangles").arg(static_cast<qulonglong>(standardMeshNormals.vertexCount())).arg(static_cast<qulonglong>(standardMeshNormals.triangleCount()));
        ...
        {
            qCritical() << "Volume-field render diagnostic failed:" << exception.what();
        }
        catch (...)
        {
            qCritical() << "Volume-field render diagnostic failed with an unknown exception.";
        }

        statusBar()->showMessage(QStringLiteral("Render diagnostic initialization failed."));
        return false;
    }
    ...
    void createToolBars()
    {
        ...
        addVisibilityAction(objectToolBar, QStringLiteral("体素"), m_voxelObject);
        addVisibilityAction(objectToolBar, QStringLiteral("标准梯度"), m_standardObject);
        addVisibilityAction(objectToolBar, QStringLiteral("标准网格法线"), m_standardMeshNormalObject);
        addVisibilityAction(objectToolBar, QStringLiteral("QEF"), m_featureObject);
        ...
        addRenderModeAction(renderToolBar, renderGroup, QStringLiteral("纯色"), DiagnosticRenderMode::Unlit, QStringLiteral("纯色无光照"));
        addRenderModeAction(renderToolBar, renderGroup, QStringLiteral("漫反射"), DiagnosticRenderMode::SmoothDiffuse, QStringLiteral("平滑顶点法线，仅漫反射"));
        QAction* specularAction = addRenderModeAction(renderToolBar, renderGroup, QStringLiteral("高光"), DiagnosticRenderMode::SmoothSpecular, QStringLiteral("平滑顶点法线，漫反射和镜面高光"));
        ...
        addRenderModeAction(renderToolBar, renderGroup, QStringLiteral("法线RGB"), DiagnosticRenderMode::NormalRgb, QStringLiteral("将平滑顶点法线直接映射到RGB"));
        specularAction->setChecked(true);
    ...
    {
        statusBar()->showMessage(QStringLiteral("左至右：解析球 | 体素 | 标准梯度法线 | 标准面积加权网格法线 | QEF | Level %1 | %2")
...
private:
    ...
    std::size_t m_voxelObject = 0; // 原始体素表面对象索引。
    std::size_t m_standardObject = 0; // 标准Surface Nets原始距离梯度法线对象索引。
    std::size_t m_standardMeshNormalObject = 0; // 标准Surface Nets面积加权网格法线对象索引。
    std::size_t m_featureObject = 0; // 特征敏感QEF对象索引。
...
int main(int argc, char* argv[])
{
    ...
    RenderDiagnosticWindow window(parseMaximumLevel(argc, argv));
