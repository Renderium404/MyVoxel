...
// 表示真正被Topology_Wire句柄共享的一维拓扑Wire实体，保存规范Forward方向下的有向Edge序列。
class Topology_TWire : public Topology_TObject
{
public:
    // 使用至少一条按Topology_Vertex身份连续连接的有向Edge构造共享拓扑Wire实体。
...
private:
    std::vector<Topology_Edge> m_edges; // 规范Forward Wire按遍历顺序保存的有向Edge句柄。
