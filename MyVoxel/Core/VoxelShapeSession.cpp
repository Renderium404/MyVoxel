...
VoxelShapeSession::VoxelShapeSession(
    VoxelShape& shape,
    ...
    , m_changes(changeTrackingLevel)
{
    MYVOXEL_ASSERT_MESSAGE(
        shape.isValid(),
        "Cannot create VoxelShapeSession for an invalid VoxelShape.");

    MYVOXEL_ASSERT_MESSAGE(
        changeTrackingLevel <= shape.grid().maximumLevel(),
    ...
    MYVOXEL_ASSERT_MESSAGE(
        !shape.isDataShared(),
        "VoxelShapeSession requires exclusively owned shape data.");
}

VoxelShapeSession::VoxelShapeSession(
    VoxelShapeSession&& other)
...
const VoxelGrid& VoxelShapeSession::grid() const
{
    MYVOXEL_ASSERT_MESSAGE(
        m_shape,
        "VoxelShapeSession is not bound to a VoxelShape.");

    return m_shape->grid();
}

bool VoxelShapeSession::supportsAddress(
...
VoxelState VoxelShapeSession::state(
    const VoxelCellAddress& address) const
{
    MYVOXEL_ASSERT_MESSAGE(
        m_shape,
    ...
    MYVOXEL_ASSERT_MESSAGE(
        m_forest,
        "VoxelShapeSession forest must not be null.");
...
bool VoxelShapeSession::hasNode(
    const VoxelCellAddress& address) const
{
    MYVOXEL_ASSERT_MESSAGE(
        m_shape,
...
bool VoxelShapeSession::setState(
    const VoxelCellAddress& address,
    VoxelState stateValue)
{
    MYVOXEL_ASSERT_MESSAGE(
        m_shape,
        "VoxelShapeSession is not bound to a VoxelShape.");
    ...
    MYVOXEL_ASSERT_MESSAGE(
        stateValue == VoxelState::Empty ||
            stateValue == VoxelState::Material,
        "VoxelShapeSession::setState only accepts Empty or Material.");

    const bool changed =
        m_forest->setState(address, stateValue);
    ...
    const VoxelCellAddress& address)
{
    ...
    MYVOXEL_ASSERT_MESSAGE(
        address.level < grid().maximumLevel(),
        "The maximum-level voxel cannot be subdivided.");
    ...
    const VoxelCellAddress& address)
{
    ...
    MYVOXEL_ASSERT_MESSAGE(
        address.level < grid().maximumLevel(),
        "The maximum-level voxel has no child voxels to merge.");
...
VoxelChangeSet::DirtyCellRegion
VoxelShapeSession::createDirtyCellRegion(
    const VoxelCellIndex& rootIndex) const
{
    ...
    return VoxelChangeSet::DirtyCellRegion(
        rootIndex,
        m_changes.trackingLevel());
}

void VoxelShapeSession::recordMaterialChange(
    VoxelChangeSet::DirtyCellRegion& dirtyRegion,
    const VoxelCellAddress& address) const
{
    MYVOXEL_ASSERT_MESSAGE(
        m_shape,
    ...
    MYVOXEL_ASSERT_MESSAGE(
        supportsAddress(address),
        "Voxel material change address exceeds the shape maximum level.");

    MYVOXEL_ASSERT_MESSAGE(
        dirtyRegion.trackingLevel() == m_changes.trackingLevel(),
...
const VoxelTree* VoxelShapeSession::tree(
    const VoxelCellIndex& rootIndex) const
{
    MYVOXEL_ASSERT_MESSAGE(
        m_forest,
...
void VoxelShapeSession::setTree(
    const VoxelCellIndex& rootIndex,
    const VoxelTree& treeValue)
{
    MYVOXEL_ASSERT_MESSAGE(
        m_forest,
        "VoxelShapeSession forest must not be null.");
    ...
    const VoxelChangeSet::DirtyCellRegion& dirtyRegion)
{
    ...
    MYVOXEL_ASSERT_MESSAGE(
        dirtyRegion.rootIndex() == rootIndex,
        "Voxel tree dirty region belongs to another Root.");
...
bool VoxelShapeSession::eraseTree(
    const VoxelCellIndex& rootIndex)
{
    MYVOXEL_ASSERT_MESSAGE(
        m_forest,
...
void VoxelShapeSession::moveTreesTo(
    std::vector<VoxelForest::TreeEntry>& trees)
{
    MYVOXEL_ASSERT_MESSAGE(
        m_forest,
        "VoxelShapeSession forest must not be null.");
    ...
    m_forest->forEachRootCell(
        ...
        {
            m_changes.recordFullRootChange(
                rootAddress.index);
        });
