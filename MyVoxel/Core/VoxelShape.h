...
// 表示由体素网格、稀疏体素森林、派生距离场资源和实例空间变换组成的离散体积。
//
// 复制VoxelShape时共享体素网格、森林和距离场资源，创建VoxelShapeSession时执行Shape级写时复制。
// 空间变换由每个VoxelShape独立保存，修改变换不会复制体素数据或使局部距离场失效。
class VoxelShape
{
public:
    ...
    // 复制体素体并共享体素网格、森林和距离场资源，空间变换独立复制。
    ...
    // 检查共享数据、体素网格、距离场资源和空间变换是否有效。
    bool isValid() const;
    // 判断当前体积是否不包含任何显式根树。
    bool isEmpty() const;
    // 判断当前体积数据是否还被其他VoxelShape共享。
    bool isDataShared() const;
    // 判断当前体积是否与另一个体积共享同一份网格、森林和距离场资源。
    ...
    // 判断当前最高体素层级是否能够使用两级展开的64样本距离块。
    bool supportsVolumeField() const;
    // 返回当前体积共享的只读距离场；最高层级不足第2层时返回空指针。
    const VolumeField* volumeField() const;
    // 返回当前体积独占的可写距离场；必要时执行Shape级写时复制，最高层级不足第2层时返回空指针。
    VolumeField* editVolumeField();
...
private:
    // 保存可以在多个VoxelShape之间共享的体素网格、森林和距离场资源。
    class SharedData : public Foundation::ReferenceCounted
    {
    public:
        // 使用指定体素网格创建空材料森林和对应的全失效距离场资源。
        explicit SharedData(const VoxelGrid& gridValue);
        // 复制体素网格和森林根映射，森林内部继续共享节点池，距离场重新创建为全失效状态。
        ...
        std::unique_ptr<VolumeField> volumeField; // 当前材料状态对应的可选派生距离场资源。

    protected:
        ~SharedData() override = default;
    };

    // 当共享数据被多个体积引用时创建当前体积的独立材料副本和全失效距离场。
    void detach();
    // 将当前独占共享数据中的距离场标记为完全失效，当前网格不支持距离场时不执行操作。
    void invalidateVolumeField();

private:
    friend class VoxelShapeSession;

private:
    Foundation::RefPtr<SharedData> m_data; // 支持写时复制的网格、森林和距离场共享数据。
