...
public:
    ...
    VoxelCursorState seek(const VoxelCellAddress& address);
...
private:
    // 返回指定地址所属的第0层根地址。
    static VoxelCellAddress rootAddressOf(VoxelCellAddress address);
...
private:
    ...
    VoxelCursorState m_state; // 最近一次目标地址的逻辑状态。
