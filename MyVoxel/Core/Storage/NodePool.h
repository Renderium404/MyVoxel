...
public:
    ...
    // 分配八个连续且64字节对齐的VoxelBlock槽，不初始化槽内实际类型。
    ...
    // 返回指定物理槽中的可写NodeBlock，调用者必须保证该槽当前物理状态为Node。
    inline NodeBlock& node(VoxelIndex index);
    // 返回指定物理槽中的只读NodeBlock，调用者必须保证该槽当前物理状态为Node。
    inline const NodeBlock& node(VoxelIndex index) const;
    // 返回指定物理槽中的可写IndexBlock，调用者必须保证该槽当前物理状态为MaskLeaf。
    inline IndexBlock& index(VoxelIndex index);
    // 返回指定物理槽中的只读IndexBlock，调用者必须保证该槽当前物理状态为MaskLeaf。
    inline const IndexBlock& index(VoxelIndex index) const;
    // 返回指定已分配物理槽中的原始VoxelBlock。
    inline VoxelBlock& block(VoxelIndex index);
    // 返回指定已分配物理槽中的只读原始VoxelBlock。
