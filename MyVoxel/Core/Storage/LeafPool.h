...
public:
    ...
    // 分配一个统一leafIndex，并使用指定距离初始化对应MaskBlock和LeafBlock。
    VoxelIndex allocateBlock(float distance = 0.0f);
    // 使用指定距离重新初始化已经分配的MaskBlock和LeafBlock。
    void initBlock(VoxelIndex index, float distance);
