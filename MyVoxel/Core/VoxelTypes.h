...
using VoxelIndex = std::int32_t; // 体素存储索引类型。

const VoxelIndex InvalidVoxelIndex = (std::numeric_limits<VoxelIndex>::max)(); // 无效体素存储索引。
...
{
    Empty = 0,      // 子节点为空，不占用VoxelBlock物理槽。
    Material = 1,   // 子节点完全包含材料，不占用VoxelBlock物理槽。
    Node = 2,       // 子节点使用VoxelBlock物理槽，并按NodeBlock解释。
    MaskLeaf = 3    // 子节点使用VoxelBlock物理槽，并按IndexBlock解释后定位对应MaskBlock和LeafBlock。
};
