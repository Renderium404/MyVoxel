...
// 记录一次体素会话实际产生的结构变化根，以及材料变化覆盖的固定层级区域。
//
// VoxelChangeSet由调用者指定跟踪层级创建空结果容器，实际变化内容只由VoxelShapeSession维护。
// 修改根集合记录材料或显式结构发生变化的全部第0层根。
// 脏区域只记录材料场变化，不记录仅由split或merge产生的纯结构变化。
class VoxelChangeSet
{
public:
    using RootIndexSet = std::set<VoxelCellIndex>;

    // 保存一个第0层根中发生材料变化的固定跟踪层级体素位图。
    class DirtyCellRegion
    {
    public:
        /// 状态判断
        // 判断当前根的全部跟踪层级体素是否均被标记为材料变化。
        bool isFullRoot() const;
        // 判断指定根内局部跟踪层级体素是否被标记为材料变化。
        bool containsLocalCell(std::size_t localX,
                               std::size_t localY,
    ...
    private:
        ...
        // 为指定根和固定跟踪层级创建空材料变化区域。
        DirtyCellRegion(const VoxelCellIndex& rootIndex, VoxelLevel trackingLevel);
        // 将指定逻辑体素覆盖的全部跟踪层级体素标记为材料变化。
        void recordMaterialChange(const VoxelCellAddress& address);
        // 将当前根的全部跟踪层级体素标记为材料变化。
        void markFullRoot();
        // 累加同一根、同一跟踪层级的另一个材料变化区域。
        void accumulate(const DirtyCellRegion& other);
        // 将指定根内局部跟踪层级体素标记为材料变化。
        void addLocalCell(std::size_t localX,
                          std::size_t localY,
                          std::size_t localZ);
        // 返回指定局部体素对应的线性位索引。
        std::size_t cellBitIndex(std::size_t localX,
                                 std::size_t localY,
                                 std::size_t localZ) const;
        // 标记指定闭区间内的全部局部跟踪层级体素。
        void markLocalRange(std::size_t minimumX,
                            std::size_t maximumX,
                            std::size_t minimumY,
                            std::size_t maximumY,
                            std::size_t minimumZ,
    ...
    private:
        ...
        VoxelLevel m_trackingLevel;             // 当前材料变化区域使用的固定跟踪层级。
        std::size_t m_axisCellCount;            // 当前根单轴包含的跟踪层级体素数量。
        std::size_t m_totalCellCount;           // 当前根包含的跟踪层级体素总数量。
        std::size_t m_changedCellCount;         // 当前区域中被标记的唯一体素数量。
        bool m_fullRoot;                        // 当前是否直接表示整个根均发生材料变化。
        std::vector<std::uint64_t> m_changedWords; // 按XYZ局部顺序保存材料变化位图。
    };
    ...
    /// 记录访问
    // 返回全部发生材料或显式结构变化的第0层根索引。
    const RootIndexSet& modifiedRootIndices() const;
    // 返回指定根的材料变化脏区域，不存在时返回空指针。
...
private:
    ...
    // 记录指定地址覆盖区域发生材料变化，并同时记录所属根。
    void recordMaterialChange(const VoxelCellAddress& address);
    // 记录指定根的材料和结构均可能整体发生变化。
    void recordFullRootChange(const VoxelCellIndex& rootIndex);
    // 累加一个由会话创建的同层级Root材料变化区域。
...
private:
    VoxelLevel m_trackingLevel; // 当前变化集合使用的固定材料变化跟踪层级。
    RootIndexSet m_modifiedRootIndices; // 发生材料或显式结构变化的根索引。
    DirtyCellRegionMap m_dirtyRegions; // 各根在固定层级中的材料变化区域。
};
