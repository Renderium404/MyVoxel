...
// 每个成功修改原语都会同步写入当前VoxelChangeSet；材料实际变化还会立即使距离场整体失效。
// 纯split或merge只改变树结构，不改变材料区域，因此不会使距离场失效。
...
public:
    ...
    // 将指定地址设置为空或材料状态，不自动合并任何祖先节点；实际改变材料时立即使距离场失效。
    bool setState(const VoxelCellAddress& address, VoxelState state);

    /// 体素结构

    // 将指定材料体素细分为八个材料子体素，不改变材料区域和距离场有效性。
    bool split(const VoxelCellAddress& address);
    // 在八个直接子体素状态一致时只合并指定体素，不改变材料区域和距离场有效性。
    ...
    // 将指定逻辑体素覆盖区域记录到由当前会话创建的材料变化区域，不直接修改Shape或距离场。
    ...
    // 判断当前是否不包含任何显式根树。
    bool isEmpty() const;
    // 清空全部显式根树，并将删除的每个根记录为完整变化，同时使距离场失效。
    bool clear();
