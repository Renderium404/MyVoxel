...
BoxGeometry::BoxGeometry(double sizeX, double sizeY, double sizeZ)
    : m_sizeX(sizeX)
    , m_sizeY(sizeY)
    , m_sizeZ(sizeZ)
...
{
    MYVOXEL_ASSERT_MESSAGE(isFinitePositive(sizeX), "BoxGeometry sizeX must be finite and greater than zero.");
    MYVOXEL_ASSERT_MESSAGE(isFinitePositive(sizeY), "BoxGeometry sizeY must be finite and greater than zero.");
    MYVOXEL_ASSERT_MESSAGE(isFinitePositive(sizeZ), "BoxGeometry sizeZ must be finite and greater than zero.");
}

/// 几何参数

double BoxGeometry::sizeX() const
{
    return m_sizeX;
}

double BoxGeometry::sizeY() const
{
    return m_sizeY;
}

double BoxGeometry::sizeZ() const
{
    return m_sizeZ;
}
...
ShapeKind BoxGeometry::kind() const
{
    return ShapeKind::Box;
}

Bounds3 BoxGeometry::localBounds() const
{
    return m_bounds;
}
...
{
    MYVOXEL_ASSERT_MESSAGE(point.isFinite(), "BoxGeometry query point must be finite.");
    return m_bounds.contains(point);
}

ShapeRelation BoxGeometry::classifyLocalBounds(const Bounds3& bounds) const
{
    MYVOXEL_ASSERT_MESSAGE(bounds.isValid(), "BoxGeometry classification bounds must be valid.");
