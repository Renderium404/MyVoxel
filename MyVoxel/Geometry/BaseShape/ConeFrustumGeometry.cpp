...
ConeFrustumGeometry::ConeFrustumGeometry(double bottomRadius, double topRadius, double height)
    ...
    , m_bounds(MyMath::Vector3(-(std::max)(bottomRadius, topRadius),
               ...
               MyMath::Vector3((std::max)(bottomRadius, topRadius),
                               ...
                               height * HalfScale))
{
    MYVOXEL_ASSERT_MESSAGE(isFiniteNonNegative(bottomRadius), "ConeFrustumGeometry bottom radius must be finite and non-negative.");
    MYVOXEL_ASSERT_MESSAGE(isFiniteNonNegative(topRadius), "ConeFrustumGeometry top radius must be finite and non-negative.");
    MYVOXEL_ASSERT_MESSAGE(bottomRadius > 0.0 || topRadius > 0.0, "ConeFrustumGeometry requires at least one radius greater than zero.");
    MYVOXEL_ASSERT_MESSAGE(isFinitePositive(height), "ConeFrustumGeometry height must be finite and greater than zero.");
}

/// 几何参数

double ConeFrustumGeometry::bottomRadius() const
{
    return m_bottomRadius;
}

double ConeFrustumGeometry::topRadius() const
{
    return m_topRadius;
}

double ConeFrustumGeometry::height() const
{
    return m_height;
}

double ConeFrustumGeometry::radiusAt(double localZ) const
{
    MYVOXEL_ASSERT_MESSAGE(isFiniteValue(localZ), "ConeFrustumGeometry localZ must be finite.");
    MYVOXEL_ASSERT_MESSAGE(localZ >= -m_halfHeight && localZ <= m_halfHeight, "ConeFrustumGeometry localZ is outside the geometry height range.");
...
ShapeKind ConeFrustumGeometry::kind() const
{
    return ShapeKind::ConeFrustum;
}

Bounds3 ConeFrustumGeometry::localBounds() const
{
    return m_bounds;
}
...
bool ConeFrustumGeometry::containsLocalPoint(const MyMath::Vector3& point) const
{
    MYVOXEL_ASSERT_MESSAGE(point.isFinite(), "ConeFrustumGeometry query point must be finite.");
...
ShapeRelation ConeFrustumGeometry::classifyLocalBounds(const Bounds3& bounds) const
{
    MYVOXEL_ASSERT_MESSAGE(bounds.isValid(), "ConeFrustumGeometry query bounds must be valid.");
