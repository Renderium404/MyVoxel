#ifndef MYVOXEL_GEOMETRY_MESH_H
#define MYVOXEL_GEOMETRY_MESH_H
...
{
    MeshColor();
    MeshColor(double redValue, double greenValue, double blueValue, double alphaValue = 1.0);

    bool operator==(const MeshColor& other) const;
    bool operator!=(const MeshColor& other) const;
...
struct MeshVertex
{
    ...
    float y; // 局部空间Y坐标。
    float z; // 局部空间Z坐标。
    float normalX; // 局部空间法线X分量，零向量表示没有有效法线。
    float normalY; // 局部空间法线Y分量，零向量表示没有有效法线。
    float normalZ; // 局部空间法线Z分量，零向量表示没有有效法线。
};

// 保存局部空间索引三角网格及其可选渲染属性。
//
// 每三个连续索引组成一个三角形。
// 顶点位置和三角形索引定义网格几何；顶点法线和逐三角形颜色不参与几何有效性判断。
// 三角形颜色必须全部存在或全部不存在，不允许只为部分三角形保存颜色。
// 当前网格不保存实例变换和GPU资源，可由连续几何网格化和体素表面网格化共同使用。
class Mesh
{
public:
    /// 网格状态
    ...
    // 检查几何和可选属性是否有效，法线与颜色必须分别全部存在或全部不存在。
    bool isValid() const;
    // 判断当前非空网格是否具有完整有效的顶点法线和逐三角形颜色。
    bool isRenderable() const;
    // 判断全部顶点是否具有有限且非零的有效法线.
    bool hasValidNormals(double epsilon = 1.0e-12) const;
    // 判断全部顶点法线是否在指定误差内为单位向量。
    bool hasUnitNormals(double epsilon = 1.0e-5) const;
    // 判断当前非空网格是否具有完整且有限的逐三角形颜色。
    ...
    // 返回与三角形一一对应的只读颜色数组，没有颜色时返回空数组。
    const std::vector<MeshColor>& triangleColors() const;
    // 返回指定三角形的颜色，当前网格必须具有完整逐三角形颜色。
    const MeshColor& triangleColor(std::size_t triangleIndex) const;
    ...
    //
    // 当两个网格都包含三角形时，它们必须同时具有或同时不具有逐三角形颜色。
    ...
    // 使用三个已有顶点索引追加一个指定颜色的三角形。
    void appendTriangle(std::uint32_t index0, std::uint32_t index1,
                        std::uint32_t index2, const MeshColor& color);
    // 追加三个独立顶点组成的不包含颜色三角形。
    void appendTriangle(const MeshVertex& vertex0,
    ...
    void appendQuad(const MeshVertex& vertex0,
                    const MeshVertex& vertex1,
                    const MeshVertex& vertex2,
                    const MeshVertex& vertex3);
    // 追加一个指定颜色的四边形，并按0-1-2和0-2-3拆分为两个三角形。
    void appendQuad(const MeshVertex& vertex0,
                    const MeshVertex& vertex1,
                    const MeshVertex& vertex2,
                    const MeshVertex& vertex3,
...
private:
    ...
    std::vector<MeshColor> m_triangleColors; // 可选的逐三角形颜色，必须为空或与三角形一一对应。
};

}
}

#endif // MYVOXEL_GEOMETRY_MESH_H