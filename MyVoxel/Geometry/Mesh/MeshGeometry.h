#ifndef MYVOXEL_GEOMETRY_MESHGEOMETRY_H
#define MYVOXEL_GEOMETRY_MESHGEOMETRY_H
...
//
// MeshGeometry只执行几何有效性和退化三角形基础检查，不在构造时重复执行高成本拓扑验证。
// 调用者应在导入或建模完成阶段使用MeshValidation确认边界边、非流形边和非流形顶点。
// 当前网格法线、颜色和三角形绕序不参与内外判断；自相交检查仍由更高层验证流程负责。
class MeshGeometry : public ShapeGeometry
{
public:
    // 复制指定内部三角网格并建立空间查询数据。
    explicit MeshGeometry(const Mesh& mesh);

    // 移动指定内部三角网格并建立空间查询数据。
    explicit MeshGeometry(Mesh&& mesh);
    ...
    ShapeRelation classifyLocalBoundsFast(
        const MyMath::Vector3& center,
        const MyMath::Vector3& extent) const override;
...
#endif // MYVOXEL_GEOMETRY_MESHGEOMETRY_H
