...
{
    return isValid() && m_signedDistanceMetricValid && geometry().supportsSignedDistance();
}
...
{
    MYVOXEL_ASSERT_MESSAGE(isValid(), "Cannot query an invalid ShapeQuery.");
    MYVOXEL_ASSERT_MESSAGE(point.isFinite(), "ShapeQuery signed-distance point must be finite.");
    MYVOXEL_ASSERT_MESSAGE(supportsSignedDistance(), "ShapeQuery signed distance requires supported geometry and a rigid or uniform-scale query transform.");

    const MyMath::Vector3 localPoint = m_identityQuery ? point : m_queryToLocal.transformPoint(point);
    const double localDistance = geometry().signedDistanceLocalPoint(localPoint);
...
{
    const MyMath::Vector3 columnX(m_queryToLocal(0, 0), m_queryToLocal(1, 0), m_queryToLocal(2, 0));
    const MyMath::Vector3 columnY(m_queryToLocal(0, 1), m_queryToLocal(1, 1), m_queryToLocal(2, 1));
    const MyMath::Vector3 columnZ(m_queryToLocal(0, 2), m_queryToLocal(1, 2), m_queryToLocal(2, 2));
    const double lengthSquaredX = MyMath::Vector3::dot(columnX, columnX);
    const double lengthSquaredY = MyMath::Vector3::dot(columnY, columnY);
    const double lengthSquaredZ = MyMath::Vector3::dot(columnZ, columnZ);
    const double maximumLengthSquared = (std::max)(1.0, (std::max)(lengthSquaredX, (std::max)(lengthSquaredY, lengthSquaredZ)));
    const double tolerance = std::numeric_limits<double>::epsilon() * SignedDistanceMetricToleranceScale * maximumLengthSquared;
    const bool equalScale = std::fabs(lengthSquaredX - lengthSquaredY) <= tolerance && std::fabs(lengthSquaredX - lengthSquaredZ) <= tolerance;
    const bool orthogonal = std::fabs(MyMath::Vector3::dot(columnX, columnY)) <= tolerance &&
                            std::fabs(MyMath::Vector3::dot(columnX, columnZ)) <= tolerance &&
                            std::fabs(MyMath::Vector3::dot(columnY, columnZ)) <= tolerance;

    m_signedDistanceMetricValid = equalScale && orthogonal && lengthSquaredX > 0.0;
    m_localDistancePerQueryUnit = m_signedDistanceMetricValid ? std::sqrt((lengthSquaredX + lengthSquaredY + lengthSquaredZ) / 3.0) : 1.0;
}
