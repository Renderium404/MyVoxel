...
public:
    ...
    void relation(MyVoxel::ShapeRelation)
    {
    }
...
public:
    ...
    void relation(MyVoxel::ShapeRelation relation)
    {
...
MyMath::Vector3 classifyChildBoundsFast(
    ...
    std::array<MyVoxel::ShapeRelation, MyVoxel::ShapeQuery::OctantCount>& relations,
    Recorder& recorder)
{
...
// 对最高层相交体素执行中心采样。
template<typename Recorder>
bool sampleCellCenter(const MyVoxel::ShapeQuery& query,
...
// 返回一个最高层体素的材料状态。
template<typename Recorder>
bool maximumLevelCellMaterial(const MyVoxel::ShapeQuery& query,
                              const CellBounds& bounds,
...
template<typename Recorder>
std::uint64_t buildMaskLeafMaterialMask(const MyVoxel::ShapeQuery& query,
                                        ...
                                        Recorder& recorder)
{
    ...
    for (unsigned int coarseIndex = 0; coarseIndex < static_cast<unsigned int>(MyVoxel::VoxelCornerCount); ++coarseIndex)
    {
        ...
        const MyVoxel::ShapeRelation coarseRelation = coarseRelations[coarseIndex];
        ...
        for (unsigned int fineIndex = 0; fineIndex < static_cast<unsigned int>(MyVoxel::VoxelCornerCount); ++fineIndex)
        {
            ...
            const MyVoxel::ShapeRelation fineRelation = fineRelations[fineIndex];
                                 ...
                                 Recorder& recorder)
{
    ...
    MYVOXEL_ASSERT_MESSAGE(relation == MyVoxel::ShapeRelation::Intersecting,
...
template<typename Recorder>
void collectRootVoxelizationItems(const MyVoxel::ShapeQuery& query,
         ...
         ++z)
    {
             ...
             ++y)
        {
                 ...
                 ++x)
            {
                ...
                const MyVoxel::ShapeRelation relation =query.classifyBoundsFast(rootBounds.center, rootBounds.extent);
...
// 并行生成全部边界相交根树。
template<typename Recorder>
void executeIntersectingRoots(const MyVoxel::ShapeQuery& query,
                              MyVoxel::VoxelLevel maximumLevel,
                              std::vector<RootVoxelizationItem>& items)
{
    ...
    MyVoxel::Foundation::ParallelExecutor::global().execute(
        ...
        {
            // ShapeQuery构造后只读且明确不可复制，多个根任务共享当前const查询器。
            for (std::size_t itemIndex = blockBegin; itemIndex < blockEnd; ++itemIndex)
            {
                processIntersectingRoot<Recorder>(items[itemIndex], query, maximumLevel);
            }
        },
        options);
}
...
template<typename Recorder>
void voxelizeQueryInto(const MyVoxel::ShapeQuery& query,
                       const MyVoxel::VoxelGrid& grid,
...
// 执行一次性统一体素化。
template<typename Recorder>
MyVoxel::VoxelShape voxelizeQueryImpl(const MyVoxel::ShapeQuery& query,
                                      const MyVoxel::VoxelGrid& grid,
                                      const MyMath::Matrix4& resultTransform,
                                      Recorder& recorder)
{
...
// 使用可复用工作区执行统一对齐体素化。
template<typename Recorder>
const MyVoxel::VoxelShape& voxelizeQueryImpl(const MyVoxel::ShapeQuery& query,
...
VoxelShape voxelize(const Topology_Shape& topology, const VoxelGrid& grid)
{
    ...
    return voxelizeQueryImpl(query, grid, MyMath::Matrix4::identity(), recorder);
}
...
VoxelShape voxelize(const Shape& shape, const VoxelGrid& localGrid)
{
    ...
    DisabledVoxelizationRecorder recorder;
    return voxelizeQueryImpl(query, localGrid, shape.localToWorld(), recorder);
}
...
VoxelShape voxelizeAligned(const Shape& shape, const VoxelShape& reference)
{
    ...
    DisabledVoxelizationRecorder recorder;
    return voxelizeQueryImpl(query, reference.grid(), reference.transform(), recorder);
}
